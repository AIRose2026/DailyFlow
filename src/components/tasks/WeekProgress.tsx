...
export function WeekProgress() {
  ...
  const { days, weekLabel, loading } = useWeekOverview(weekOffset);
      ...
      <div className="mb-3 flex items-center justify-between">
        ...
        <div className="flex items-center gap-1">
          <button
            ...
            className="flex h-7 w-7 items-center justify-center rounded-full text-white/40 transition-colors hover:text-white"
          >
            <ChevronLeft size={16} />
          </button>
          <button
            ...
            className="min-w-[6.5rem] text-center text-xs text-white/40 disabled:cursor-default"
          >
            {weekOffset === 0 ? weekLabel : `${weekLabel} · Zu heute`}
          </button>
          <button
            type="button"
            onClick={() => goToWeek(weekOffset + 1)}
